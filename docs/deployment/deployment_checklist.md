...
使用 `scripts/run-api-smoke.ps1` 或平台等价方式依次验证真实登录、当前用户信息、案例列表、评分提交、训练记录、知识问答和退出登录。使用两个不同学员账号确认：不能读取对方训练记录，不能复用对方会话或提交结果。
...
确认停止期间应用能在 30 秒内完成优雅退出，重启后数据库连接、登录、评分和问答仍正常。日志只保留必要运行信息，不得出现 API Key、密码、Cookie、完整学员答案、模型原文或内部地址。
