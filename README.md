...
面向托管业务新人的桌面端学习产品。用户登录后从清算、核算、监督三个学习世界中进入所属条线；当前核算世界已正式开放岗位导入“站上核算岗”和完整的股票核算业务模块。
...
- `ACCOUNTING`：开放；正式内容包含 `ACC-LIFE-ROLE-001`“站上核算岗”，以及股票模块 4 条必修路线和 4 条非阻挡进阶路线。
- `CLEARING`、`SUPERVISION`：内容建设中，不提供虚假路线或进度。
- “接管一只新产品”仍仅展示为建设中节点；股票业务线不以该节点作为前置。股票必修主链为普通买卖与成本、网上新股、现金分红与送转、股息红利税，进阶支线为非公开发行、受限估值、港股通交易和港股通现金分红。
