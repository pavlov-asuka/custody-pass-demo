...
const FAILING_ANSWER = '事实';
const PASSING_ANSWER = `我作为组合核算人员仍对结果负责。系统执行成功不代表业务结果正确，不能替代人工核验。
...
async function answerCurrentPractice(page) {
  ...
  if (prompt.includes('哪些做法') || prompt.includes('符合核算岗')) {
    await answerChoiceByKeys(page, ['A', 'B', 'D']);
    return 'MULTIPLE';
  }
...
async function completeBasicPractice(page) {
  ...
  await page.waitForSelector('#formal-answer, .exception-case', { timeout: 20000 });
}

async function submitException(page, answer) {
  await page.waitForSelector('#formal-answer', { timeout: 15000 });
  await page.fill('#formal-answer', answer);
  await page.waitForTimeout(1100);
  await page.locator('button.button--primary').filter({ hasText: '提交答案' }).click();
...
async function completeAllRemediation(page) {
  ...
  await page.waitForSelector('button.button--primary:has-text("重新挑战完整异常案例")', { timeout: 20000 });
  check('全部补学完成后出现重新挑战按钮', true);
  await clickPrimaryByText(page, '重新挑战完整异常案例');
...
async function run() {
  ...
  await shot(page, '07-exception-before-fail');

  const firstAttemptId = await submitException(page, FAILING_ANSWER);
  ...
  await shot(page, '11-exception-after-remediation');
  ...
  const secondAttemptId = await submitException(page, PASSING_ANSWER);
  ...
  check('API 第一条答案快照未变', apiHistory.first.answerSnapshot === FAILING_ANSWER);
